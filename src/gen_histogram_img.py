...
class GenerateHistogram(CommonTransformations):
    ...
    def create_and_show_img(self):
        # Define blocks to tear the image
        block_size = 16

        # Tear image into block_size x block_size non overlapping blocks
        teared_up_image = super(GenerateHistogram, self).break_image(self.image,
                                                                     block_size)

        # Create an empty(with zeros) matrix of size block_size x block_size
        teared_up_image_with_histograms = np.zeros((block_size, block_size),
                                                   dtype=object)

        # Compute the histogram of the corresponding pixel at teared_up_image
...
def im_hist(image, bins):
    # Array of size of bins (usually 256)
